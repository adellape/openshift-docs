...
The `apb` CLI tool helps APB authors create, build, and publish their APBs to
container registries. It enforces best practices and takes care of the details
so they should be easy to deploy.
...
Push to the local OpenShit Container Registry:

----
$ apb push -o
----

Push to the local OpenShit Container Registry under namespace `myproject`:
