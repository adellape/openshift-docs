...
. If any Operator or Operand images that are referenced by a subscribed Operator require access to a private registry, you can either provide access to all namespaces in the cluster, or target individual namespaces.
...
* To update individual namespaces, add your pull secret to the `default` service account in each namespace that you want to have access:
+
[source,terminal]
----
$ oc secrets link -n <namespace> \
    default <secret_name> \//<1>
    --for=pull
----
<1> If you have created a dedicated service account that pods in the namespace use instead of `default`, replace `default` with that service account name.
